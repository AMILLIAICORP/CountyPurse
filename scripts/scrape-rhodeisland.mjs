...
const COUNTIES = [
  { name: 'Providence', url: 'https://www.providenceri.gov/finance/budget' },
  { name: 'Kent', url: 'https://www.kentcountyri.gov/finance/budget' },
  { name: 'Washington', url: 'https://www.washingtonri.gov/finance/budget' },
  { name: 'Newport', url: 'https://www.newportri.gov/finance/budget' },
  { name: 'Bristol', url: 'https://www.bristolri.gov/finance/budget' },
]
async function processCounty(county) {
  console.log('Processing ' + county.name + ' County, RI...')
  try {
...
async function main() {
  console.log('CountyPurse — Rhode Island Budget Scraper')
